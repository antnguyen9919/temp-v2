...
const renderClient = row => {
  if (row.avatar.length) {
    return (
      <AvatarWithImageLink href={`/management/member/view/${row.id}`}>
        <CustomAvatar src={row.avatar} sx={{ mr: 3, width: 30, height: 30 }} />
      </AvatarWithImageLink>
    )
  } else {
    return (
      <AvatarWithoutImageLink href={`/management/member/view/${row.id}`}>
        <CustomAvatar
          skin='light'
...
const RowOptions = ({ id }) => {
  ...
  return (
    <>
      ...
      >
        <MenuItem sx={{ p: 0 }}>
          <Link href={`/management/member/view/${id}`} passHref>
            <MenuItemLink>
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        {/* <>
          <PageHeader
            title={<Typography variant='h5'>List of Members</Typography>}
            subtitle={
